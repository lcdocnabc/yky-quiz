...
const TYPES = ["填空", "单选", "多选"];
  ...
  function parseCsv(text) {
    ...
    const isHead = (head.includes("题型") || head.includes("知识类型")) && (head.includes("题干") || head.includes("答案"));
    if (isHead) {
      start = 1;
      if (head.includes("分类")) mode = 5; // 旧格式：分类,题型,题干,答案,选项
    }
    const out = [];
    for (let k = start; k < rows.length; k++) {
      const r = rows[k];
      let type, q, a, opt;
      if (mode === 5 && r.length >= 5) {
  ...
  window.YKY = {
    ...
    csvCell, parseCsv, parseXlsx, toCsv, parseImportText,
